...
class FirebaseLoginScreen(Screen, EventDispatcher):
    ...
    user has logged in (write that code in the on_login_success layout shown
    ...
    def show_password(self):
        self.count+=1
        if self.count % 2  == 0:
            self.ids.sign_in_screen.ids.password.password = True
        else:
            self.ids.sign_in_screen.ids.password.password = False
